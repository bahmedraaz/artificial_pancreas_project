...
#################################### Context table check #################################################################
	...
	if glucose < 39:
		gt_target_visit_stat = 1
		lt_target_rising_visit_stat = 1
		lt_target_falling_visit_stat = 1
	...
	if glucose >= 75:
		...
		if e_bg > bg_target:
			...
			if short_avgdelta - prev_short_avgdelta > 0:
				if recommended_change_rate >= 0 or loaded_suggested_data["rate"] == basal:
					...
					if recommended_change_rate >= -0.20:
						...
						loaded_suggested_data["fault"] = "yes"
						loaded_suggested_data["fault_reason"] = "Reason: glucose > 75; e_bg > bg_target; e_bg is rising; recommended_rate decreases" 
						print("\n***************************************")
						print("********** Faultiy !!!!! *************")  	
						print("Reason: glucose > 75; e_bg > bg_target; e_bg is rising; recommended_rate decreases")
						print("***************************************\n")
						fault_occurrence += 1
		...
		if e_bg < bg_target:
			...
			#	lt_target_rising_visit_stat = 0
				if recommended_change_rate < 0:
					track_decrease["num_of_decrease"] +=1
					if track_decrease["num_of_decrease"] == 1:
						track_decrease["iteration"] = _

					if track_decrease["num_of_decrease"] > 1 and _ == track_decrease["iteration"] + 1:
						loaded_suggested_data["fault"] = "yes"
						loaded_suggested_data["fault_reason"] = "e_bg > 75; e_bg < bg_target; bg is rising; rate should not decrease, but it decreases twice "
						print("\n***************************************")
						print("********** Faultiy !!!!! *************") 
						...
						track_decrease["num_of_decrease"] = 0
						track_decrease["iteration"] = 0

					elif track_decrease["num_of_decrease"] > 1 and _ != track_decrease["iteration"] + 1:
						track_decrease["num_of_decrease"] == 1
						track_decrease["iteration"] == _
						
				elif recommended_change_rate == 0:
				#	if lt_target_rising_visit_stat == 0 and track_decrease["iteration"] != 0:
				#		track_decrease["iteration"] += 1
				#	elif lt_target_rising_visit_stat == 1:
				#		track_decrease["num_of_decrease"] = 0
				#		track_decrease["iteration"] = 0
					if _ == track_decrease["iteration"] + 1:
						track_decrease["iteration"] = _
					else:
						track_decrease["num_of_decrease"] = 0
						track_decrease["iteration"] = 0
			...
			#	lt_target_rising_visit_stat = 0
				if recommended_change_rate > 0:
					track_increase["num_of_increase"] +=1
					if track_increase["num_of_increase"] == 1:
						track_increase["iteration"] = _

					if track_increase["num_of_increase"] > 1 and _ == track_increase["iteration"] + 1:
						loaded_suggested_data["fault"] = "yes"
						loaded_suggested_data["fault_reason"] = "e_bg > 75; e_bg < bg_target; "
						print("\n***************************************")
						print("********** Faultiy !!!!! *************") 
						print("Reason: e_bg > 75; e_bg < bg_target; bg is falling; rate should not increase, but it decrease twice") 	
						print("***************************************\n")
						fault_occurrence += 1
	
						track_increase["num_of_increase"] = 0
						track_increase["iteration"] = 0

					elif track_increase["num_of_increase"] > 1 and _ != track_increase["iteration"] + 1:
						track_increase["num_of_increase"] == 1
						track_decrease["iteration"] == _
				...
				#		track_decrease["num_of_decrease"] = 0
				#		track_decrease["iteration"] = 0
					if _ == track_increase["iteration"] + 1:
						track_increase["iteration"] = _
					else:
						track_increase["num_of_increase"] = 0
						track_increase["iteration"] = 0
			...
			#			track_increase["iteration"] = _
			#			#track_increase = {"iteration": 0, "num_of_increase":0} 
			#		if track_increase["num_of_increase"] == 2 and _ == track_increase["iteration"] + 1:	
			#			loaded_suggested_data["fault"] = "yes"	
			#			loaded_suggested_data["fault_reason"] = "e_bg > 75; e_bg < bg_target; bg is falling; still rate is increased twice"
			#			print("\n***************************************")
			#			print("********** Faultiy !!!!! *************")
			#			print("e_bg > 75; e_bg < bg_target; bg is falling; still rate is increased twice") 
			#			print("***************************************\n")
	...
	prev_e_bg = e_bg
	prev_short_avgdelta = short_avgdelta
	prev_bgi = bgi
